/**
 * Knowledge skill: search, get, sync, add, learn, learn_from_urls, list_topics, list_points, skill_create.
 * Workspace: agent dir (default ./.first_paramecium) or KNOWLEDGE_WORKSPACE. Files: KNOWLEDGE.md, knowledge/*.md.
 * Optional FTS5 index at knowledge/index.sqlite (Node 22+); optional hybrid embedding search (EMBEDDING_API_KEY).
 */
...
/** Path safety: disallow .. and backslashes; used for topic/point path segments. */
...
/** Append content to knowledge/<topic>/<point>.md, knowledge/<topic>/learned.md, or KNOWLEDGE.md. */
...
/** Strip HTML to plain text: remove script/style, then tags. */
...
/** Recursively collect all .md under a dir; relBase is path prefix relative to workspace. */
...
/** When topic is set, keep only paths that are knowledge/<topic>.md or under knowledge/<topic>/. */
...
/** topic as dir or single file: exact = knowledge/<topic>.md, prefix = knowledge/<topic>/ (for LIKE, escape % _). */
...
export const tools: AgentTool[] = [
	{
		...
		description:
			"Search KNOWLEDGE.md and knowledge/*.md by keyword. Optional topic restricts to that theme (e.g. stock, faq). Call before answering from docs/FAQ/knowledge-base. Uses FTS5 index if present, else file scan.",
		parameters: {
			type: "object",
			properties: {
				query: { type: "string", description: "Search phrase" },
				topic: { type: "string", description: "Optional; restrict to this topic, e.g. stock, faq (knowledge/<topic>.md or knowledge/<topic>/)" },
	...
	{
		name: "knowledge_get",
		description: "Read a slice of a knowledge file by path and optional line range. Use after knowledge_search when needed. Paths allowed: KNOWLEDGE.md, knowledge.md, knowledge/*.md, KNOWLEDGE_EXTRA_PATHS.",
		parameters: {
			type: "object",
			properties: {
				path: { type: "string", description: "Path relative to workspace, e.g. KNOWLEDGE.md, knowledge/faq.md" },
	...
	{
		name: "knowledge_sync",
		description: "Rebuild FTS5 index (.first_paramecium/knowledge/index.sqlite). Call after bulk edits to improve knowledge_search. Requires Node 22+.",
		parameters: { type: "object", properties: {} },
	},
	{
		name: "knowledge_add",
		description: "Append one item to KNOWLEDGE.md ## Add block or to a given path (e.g. knowledge/faq.md). Use for 'remember this' / single facts.",
		parameters: {
			type: "object",
			properties: {
				text: { type: "string", description: "Content to store" },
				path: { type: "string", description: "Optional path, e.g. knowledge/faq.md; omit to append to KNOWLEDGE.md ## Add" },
			},
			required: ["text"],
		},
	},
	{
		...
		description: "Learn: write text into the knowledge base. topic = folder (e.g. stock, math), point = subpath (e.g. K-line, functions). topic+point → knowledge/<topic>/<point>.md; topic only → knowledge/<topic>/learned.md; else KNOWLEDGE.md. Run knowledge_sync after learning.",
		parameters: {
			type: "object",
			properties: {
				text: { type: "string", description: "Text to learn" },
				topic: { type: "string", description: "Optional topic (folder name), e.g. stock, math" },
				point: { type: "string", description: "Optional point (subpath), e.g. K-line, tech/patterns" },
				source: { type: "string", description: "Optional source label, e.g. url or conversation" },
			},
			required: ["text"],
		},
	},
	{
		...
		description: "List topics in the knowledge base: .md filenames (without .md) and subdirs under knowledge/, e.g. stock, faq. Use to pick a topic or decide whether to create a topic skill.",
		parameters: { type: "object", properties: {} },
	},
	{
		name: "knowledge_list_points",
		description: "List points under a topic (files and subdirs under knowledge/<topic>/). e.g. topic=stock returns [K-line, fundamentals, tech/]. Use to see topic structure.",
		parameters: {
			type: "object",
			properties: {
				topic: { type: "string", description: "Topic name (folder), e.g. stock, math" },
			},
			required: ["topic"],
		},
	},
	{
		...
		description: "Fetch URLs and write content into the knowledge base (optional topic/point). For 'learn from the web': use web_search first, then this tool to persist into knowledge.",
		parameters: {
			type: "object",
			properties: {
				urls: { type: "array", items: { type: "string" }, description: "URLs to fetch" },
				topic: { type: "string", description: "Optional topic (folder)" },
				point: { type: "string", description: "Optional point (subpath)" },
				maxContentPerUrl: { type: "number", description: "Max chars per URL, default 30000" },
			},
			required: ["urls"],
		},
	},
	{
		...
		description: "Turn a topic into a dedicated skill. Creates .first_paramecium/skills/<topic>_knowledge/; next run registers <topic>_knowledge_search for that topic only. e.g. topic=stock → stock_knowledge skill.",
		parameters: {
			type: "object",
			properties: {
				topic: { type: "string", description: "Topic name, e.g. stock, faq; creates <topic>_knowledge dir" },
				description: { type: "string", description: "Optional skill description, e.g. stock, K-line, fundamentals" },
			},
			required: ["topic"],
		},
	},
...
export async function executeTool(
	...
	try {
		...
		if (name === "knowledge_skill_create") {
			...
			const skillMd = `---
name: ${skillDirName}
description: "Search the «${skillDesc}» knowledge base only. Generated by knowledge_skill_create; use when the user asks about ${skillDesc}. Call ${topic}_knowledge_search then knowledge_get as needed."
---

# ${skillDirName}

When the user asks about **${skillDesc}**, use **${topic}_knowledge_search** in this topic's knowledge base, then **knowledge_get** for full snippets.

## Tools

- **${topic}_knowledge_search(query, maxResults?)**: Search only under knowledge/${topic}/ (this topic).
`;
