...
export function computeJobNextRunAtMs(job: CronJob, nowMs: number): number | undefined {
	...
	if (job.schedule.kind === "at") {
		if (job.state.lastStatus === "ok" && job.state.lastRunAtMs != null) return undefined;
		const atMs = parseAbsoluteTimeMs(job.schedule.at);
		return atMs ?? undefined;
	}
	if (job.schedule.kind === "every") {
		const anchorMs = resolveEveryAnchorMs(job.schedule, job.createdAtMs ?? nowMs);
...
export function createJob(nowMs: number, input: CronJobCreate): CronJob {
	...
	const job: CronJob = {
		...
		deleteAfterRun:
			typeof input.deleteAfterRun === "boolean" ? input.deleteAfterRun : schedule.kind === "at" ? true : undefined,
		createdAtMs: nowMs,
		updatedAtMs: nowMs,
		schedule,
...
export function applyJobPatch(job: CronJob, patch: CronJobPatch): void {
	...
	if (patch.description !== undefined) job.description = normalizeText(patch.description);
...
export function nextWakeAtMs(jobs: CronJob[], nowMs: number): number | null {
	...
	return Math.min(...enabled.map((j) => j.state.nextRunAtMs as number));
}
