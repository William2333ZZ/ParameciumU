...
import { applyJobPatch, computeJobNextRunAtMs, createJob, findJob, nextWakeAtMs, recomputeNextRuns } from "./jobs.js";
import { loadStore, saveStore } from "./store.js";
import type { CronJob, CronJobCreate, CronJobPatch, CronStoreFile } from "./types.js";

/**
 * Resolve store path: CRON_STORE env or default under current project .first_paramecium (./.first_paramecium/cron/jobs.json).
 */
export function getDefaultStorePath(cwd: string = process.cwd()): string {
	const env = process.env.CRON_STORE?.trim();
	if (env) return path.resolve(env);
	return path.join(cwd, ".first_paramecium", "cron", "jobs.json");
}
...
export class CronStore {
	...
	async list(opts?: { includeDisabled?: boolean }): Promise<CronJob[]> {
		...
		return [...filtered].sort((a: CronJob, b: CronJob) => (a.state.nextRunAtMs ?? 0) - (b.state.nextRunAtMs ?? 0));
	}
	...
	> {
		...
		const now = Date.now();
		const due =
			mode === "force" || (job.enabled && typeof job.state.nextRunAtMs === "number" && now >= job.state.nextRunAtMs);
		...
		if (job.schedule.kind === "at" && job.deleteAfterRun === true && job.state.lastStatus === "ok") {
			store.jobs = store.jobs.filter((j) => j.id !== id);
		}
		await saveStore(this.storePath, store);
