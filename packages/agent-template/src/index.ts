...
/**
 * 返回工作区下 agent 目录路径（默认 ./.first_paramecium）。
 */
export function getAgentDir(rootDir: string = process.cwd()): string {
	return path.join(path.resolve(rootDir), DEFAULT_AGENT_DIR);
}

export interface EnsureAgentDirOptions {
	/** 工作区根目录，默认 process.cwd()；与 agentDir 二选一 */
	rootDir?: string;
	/** 直接指定 agent 目录（与 .first_paramecium 同构）；与 rootDir 二选一 */
	agentDir?: string;
...
export function ensureAgentDir(options: EnsureAgentDirOptions = {}): string {
	...
	const agentDir = options.agentDir != null ? path.resolve(options.agentDir) : getAgentDir(rootDir);
	const templateDir = getPackageTemplateDir();

	if (!existsSync(templateDir)) {
		throw new Error(
			`@monou/agent-template: template not found at ${templateDir}. Run from ParameciumU monorepo or reinstall.`,
		);
	}
 ...
 */
export function getAgentSkillDirs(rootOrAgentDir: string = process.cwd(), opts?: { asAgentDir?: boolean }): string[] {
