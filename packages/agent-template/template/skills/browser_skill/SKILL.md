...
| **browser_fetch** | 打开 URL 取正文+截图；或传 **currentPageOnly: true** 不跳转、仅对当前页截图取正文（弹窗/私信框出现后用来截屏）。参数：url 或 currentPageOnly、waitAfterLoadMs、waitUntil 等。 |
| **browser_links** | 获取**当前页面**所有链接（文本 + URL），便于找到「我的主页」等再打开或点击。 |
| **browser_click** | 在**当前页面**点击元素：按文本（如「我的主页」「发私信」）或按 CSS 选择器。 |
...
- **「打开某网页、把页面内容/截图给我」** → `browser_fetch`（传 url）
- **弹窗/私信框要截图** → 先 `browser_click` 打开弹窗（如「发私信」），再 `browser_fetch({ currentPageOnly: true })` 截当前页（含弹窗），然后 `browser_fill` 填内容、`browser_click` 点发送。
