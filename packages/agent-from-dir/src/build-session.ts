...
export async function buildSessionFromU(
  ...
  const codeSkillExecuteTool = codeSkillModule.executeTool as (name: string, args: Record<string, unknown>) => Promise<{ content: string; isError?: boolean }>;

  const todoSkillModule = await loadSkillModule(toolsPath("todo_skill"));
  const todoSkillTools = todoSkillModule.tools;
  const todoSkillExecuteTool = todoSkillModule.executeTool as (name: string, args: Record<string, unknown>) => Promise<{ content: string; isError?: boolean }>;
  ...
  const { tools: scriptTools, entries: scriptEntries } = loadSkillScriptTools(skillDirs, {
    excludeDirNames: ["base_skill", "code_skill", "todo_skill", "skill-creator", "memory", "knowledge", "cron", "web_skill", "browser_skill", "message_skill", "sessions_skill", "gateway_skill"],
  });
