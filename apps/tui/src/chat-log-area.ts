	...
	private renderMessage(msg: Message, width: number): string[] {
		...
		if (msg.role === "user") {
			...
			for (let i = 0; i < Math.min(wrapped.length, MAX_LINES_PER_MESSAGE); i++) {
				out.push(i === 0 ? theme.userLabel() + theme.userText(wrapped[i]!) : indent + theme.userText(wrapped[i]!));
			}
			if (wrapped.length > MAX_LINES_PER_MESSAGE) out.push(theme.dim(indent + "..."));
		} else if (msg.role === "assistant") {
			const wrapped = wrapTextWithAnsi(msg.content, contentWidth);
			for (let i = 0; i < Math.min(wrapped.length, MAX_LINES_PER_MESSAGE); i++) {
				out.push(i === 0 ? theme.assistantLabel() + theme.fg(wrapped[i]!) : indent + theme.fg(wrapped[i]!));
			}
			if (wrapped.length > MAX_LINES_PER_MESSAGE) out.push(theme.dim(indent + "..."));
			if (msg.toolCalls?.length) {
				const line = msg.toolCalls.map((t) => `${t.name}(${t.arguments ?? ""})`).join(", ");
				out.push(theme.dim("   • ") + theme.toolTitle(truncateToWidth(line, width - 4, "")));
			}
		} else {
