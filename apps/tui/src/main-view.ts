...
import { Container, truncateToWidth } from "@monou/tui";
import type { TUI } from "@monou/tui";
import { matchesKey } from "@monou/tui";
...
export class MainView extends Container implements Component {
  ...
  constructor(
    ...
    this.chatPanel = new ChatPanel(tui, gw, { onSwitchToCron: () => this.setTab("cron") }, {
  ...
  private cronSelectPrevAgent(): void {
    ...
    this.cronSelectedAgentIndex = (this.cronSelectedAgentIndex - 1 + this.cronAgentIds.length) % this.cronAgentIds.length;
  ...
  async refreshCronJobs(): Promise<void> {
    try {
      ...
      const res = (await this.gw.call("cron.list", { agentId, includeDisabled: true }, 10_000)) as { jobs?: unknown[] };
