...
import { Container, Editor, Loader, Text, type TUI, truncateToWidth } from "@monou/tui";
...
export class ChatPanel extends Container implements Component {
	...
	private renderStatus(): void {
		...
		if (busy) {
			...
			this.statusLoader?.setMessage(this.statusError ? this.statusError : "发送中…");
		} else {
			this.statusLoader?.stop();
			this.statusLoader = null;
	...
	private updateFooter(): void {
		const sessionLabel = formatSessionKey(this.sessionKey);
		const agentLabel = this.defaultAgentId || ".u";
		this.footerText.setText(theme.dim(`agent ${agentLabel} | session ${sessionLabel}`));
	}
	...
	private async handleSubmit(raw: string): Promise<void> {
		...
		try {
			...
			if (last && last.role === "assistant") {
				...
				if (!last.content && last.toolCalls?.length) last.content = "(已执行工具)";
			}
			if (!this.sessionKey) {
