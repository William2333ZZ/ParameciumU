...
export function registerInboundHandler(
	...
	dispatcher.register({
		"im.message.receive_v1": async (data: unknown) => {
			log(
				`[feishu] event received (raw keys: ${typeof data === "object" && data !== null ? Object.keys(data as object).join(",") : "null"})`,
			);
			...
			if (!text || text === "[非文本消息]") {
				log(
					`[feishu] skip non-text message chat=${chatId} type=${messageType} content_sample=${String(message.content).slice(0, 80)}`,
				);
				return;
			}
			...
			try {
				...
				log(
					`[feishu] error: ${e.message}${e.response?.data ? ` response=${JSON.stringify(e.response.data).slice(0, 300)}` : ""}`,
				);
			}
		},
	});
