...
import type { GatewayEvent, GatewayRequest, GatewayResponse } from "@monou/gateway";
import { GATEWAY_METHODS } from "@monou/gateway";
import type { WebSocket } from "ws";
import { WebSocketServer } from "ws";
import type { AuthConfig } from "./auth.js";
import { isAuthRequired, verifyConnect } from "./auth.js";
import type { ConnectionEntry, GatewayContext } from "./context.js";
		...
		ws.on("message", (data: Buffer | ArrayBuffer) => {
			...
			if (method === CONNECT_METHOD && params && typeof params === "object") {
				...
				if (entry.identity?.role === "agent") {
					...
					for (const [existingConnId, existingEntry] of context.connections) {
						...
						if (
							existingEntry.identity?.role === "agent" &&
							existingEntry.identity.agentId === agentId &&
							existingEntry.ws.readyState === 1
						) {
							console.error(
								`[gateway] agent duplicate closed: agentId=${agentId} oldConnId=${existingConnId} (one agent one connection)`,
							);
							try {
					...
					console.error(
						`[gateway] agent connected: agentId=${entry.identity.agentId} deviceId=${entry.identity.deviceId} connId=${connId}`,
					);
				}
				if (entry.identity?.role === "connector" && entry.identity?.connectorId) {
					console.error(
						`[gateway] connector connected: connectorId=${entry.identity.connectorId} connId=${connId}`,
					);
				}
			...
			if (authRequired && !entry.authenticated) {
				send(ws, {
					id,
					ok: false,
					error: { code: UNAUTHORIZED, message: "Send connect with token/password first" },
				});
				ws.close();
				return;
			}
			...
			const handler = (
				handlers as Record<
					...
					(
						...
						req?: { connId: string; entry: ConnectionEntry },
			...
			handler(params ?? {}, reqCtx)
				.then((res) => send(ws, { ...res, id }))
				.catch((e) => {
	...
	setInterval(() => {
		...
		for (const [connId, entry] of context.connections) {
			...
			if (now - last > timeoutMs) {
				console.error(
					`[gateway] agent heartbeat timeout: agentId=${entry.identity.agentId} connId=${connId}, closing`,
				);
				try {
					entry.ws.close();
				} catch {
...
export function pushToConnector(context: GatewayContext, connectorId: string, event: string, payload: unknown): void {
