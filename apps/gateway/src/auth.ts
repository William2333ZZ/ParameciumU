...
export function verifyConnect(config: AuthConfig, params: Record<string, unknown>): { ok: boolean; reason?: string } {
  ...
  if (config.token != null && config.token !== EMPTY && providedToken != null && safeEqual(config.token, providedToken))
    return { ok: true };
  if (config.password != null && config.password !== EMPTY && providedPassword != null && safeEqual(config.password, providedPassword))
    return { ok: true };
