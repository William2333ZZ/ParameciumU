...
export type StoredMessage = {
	...
	toolCallId?: string;
	isError?: boolean;
	/** 可选：群聊中该条 assistant 的发送者 agentId */
...
/**
 * 返回 transcript 中最后一条 message 的 id（按文件顺序）；无 message 时返回 null。
 */
export function getLastMessageIdInTranscript(transcriptPath: string): string | null {
	const lines = loadTranscriptLines(transcriptPath);
	const messageEntries = lines.filter(isMessageEntry);
	if (messageEntries.length === 0) return null;
	return messageEntries[messageEntries.length - 1]!.id;
}
...
export function appendTranscriptMessages(
	...
	const existingIds = new Set(existing.filter(isMessageEntry).map((e) => e.id));
 ...
 */
export function loadTranscriptTree(transcriptPath: string): {
	header: SessionHeaderLine | null;
	entries: MessageEntryLine[];
} {
	const lines = loadTranscriptLines(transcriptPath);
	const header =
		lines.length > 0 && (lines[0] as TranscriptLine).type === "session" ? (lines[0] as SessionHeaderLine) : null;
