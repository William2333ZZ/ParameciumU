 ...
 */
export function saveScreenshotsInContent(content: string, sessionKey: string, screenshotsDir: string): string {
...
export function resolveScreenshotPath(screenshotsDir: string, sessionKey: string, id: string): string | null {
	const safe = safeSessionKey(sessionKey);
...
export function replaceAttachmentPlaceholderWithPendingUrl(content: string, screenshotsDir: string): string {
	if (!content || typeof content !== "string") return content;
