...
export function setActiveRun(state: SessionQueueState, sessionKey: string, runId: string): void {
	state.sessionActiveRun.set(sessionKey, runId);
	state.runIdToSessionKey.set(runId, sessionKey);
}

/** run 结束时根据 runId 清除并返回 sessionKey */
export function clearActiveRunByRunId(state: SessionQueueState, runId: string): string | undefined {
