...
ws.on("close", () => {
  ...
  if (!connectDone) {
    console.error("提示: 若 Gateway 启用了认证，请在 .env 中设置 GATEWAY_TOKEN 或 GATEWAY_PASSWORD 与 Gateway 一致；并确认 .env 中 GATEWAY_WS_URL 指向本项目 Gateway（npm run gateway）。");
  }
  unlock();
  process.exit(0);
});
  ...
  runScheduler(cronStorePath, {
    log: {
      info: (m: string, d?: Record<string, unknown>) =>
        console.log(`[cron] ${m}`, d ?? ""),
      warn: (m: string, d?: Record<string, unknown>) =>
        console.warn(`[cron] ${m}`, d ?? ""),
    },
