...
export async function runAgentTurn(
	...
	const result =
		opts?.onTextChunk != null
			...
			: await runAgentTurnWithTools(state, config, streamFn, message, session.executeTool, opts?.signal);

	let newLeafId: string | undefined;
	if (opts?.transcriptPath && initialMessages !== undefined) {
		const newMessages = result.state.messages.slice(initialMessages.length);
		if (newMessages.length > 0) {
			const toAppend = agentMessagesToStored(newMessages);
			newLeafId = appendTranscriptMessages(opts.transcriptPath, opts.leafId ?? null, toAppend);
		}
	}
