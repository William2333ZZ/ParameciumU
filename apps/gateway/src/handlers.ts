...
import { appendTranscriptMessages, initTranscript, loadTranscript, loadTranscriptTree } from "./session-transcript.js";
...
export function createHandlers(ctx: HandlersContext) {
	...
	return {
		...
		agent: async (params: Record<string, unknown>, req?: RequestContext): Promise<GatewayResponse> => {
			...
			const promise = new Promise<{ text: string; toolCalls?: Array<{ name: string; arguments?: string }> }>(
				(resolve, reject) => {
			...
			doRun()
				.then((result) => {
					...
					if (ctx.broadcast)
						ctx.broadcast("agent.run.done" as string, {
							runId,
							sessionKey: resolvedKey,
							text: result.text,
		...
		"chat.send": async (params: Record<string, unknown>, req?: RequestContext): Promise<GatewayResponse> => {
			...
			if (isRemote) {
				...
				if (process.env.GATEWAY_DEBUG_MEMORY) {
					process.stderr.write(
						`[gateway] chat.send remote initialMessages=${initialMessages.length} leafId=${resolvedEntry?.leafId ?? "null"}\n`,
					);
				}
				return new Promise<GatewayResponse>((resolve) => {
					const timeout = setTimeout(() => {
				...
				nodes = [
					{
						...
						deviceId: localHostname,
						connId: "",
						agents: [{ agentId: DEFAULT_LOCAL_AGENT_ID, connId: "" }],
					},
					...nodes,
				];
			}
		...
		"connector.message.inbound": async (params: Record<string, unknown>): Promise<GatewayResponse> => {
			...
			const runOneTurnLocalInbound = async (sk: string, msg: string): Promise<void> => {
				...
				if (ctx.sessionQueue) setActiveRun(ctx.sessionQueue, rkey, runId);
				try {
					const result = await ctx.runAgent!(ctx.rootDir, msg, {
						transcriptPath: tpath,
						leafId: res.entry?.leafId ?? undefined,
					});
					const patch: { updatedAt: number; leafId?: string } = { updatedAt: Date.now() };
					if (result.newLeafId != null) patch.leafId = result.newLeafId;
					updateSessionEntry(sessionStorePath, rkey, patch);
