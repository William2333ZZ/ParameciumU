...
if (!GATEWAY_URL || !AGENT_ID) {
  ...
  console.error("示例: GATEWAY_WS_URL=ws://127.0.0.1:9347 AGENT_ID=.u AGENT_DIR=./.u npm run agent");
  process.exit(1);
}
...
async function ensureHeartbeatJob(storePath: string): Promise<void> {
  ...
  console.log("[heartbeat] default job added (enabled: true). Customize in HEARTBEAT.md or .u/cron/jobs.json.");
}
...
const onFirstMessage = async (data: Buffer | ArrayBuffer) => {
  ...
  if (msg.id === "connect-1") {
    ...
    runScheduler(cronStorePath, {
      ...
      shouldRunJob: (job, nowMs) =>
        job.name !== HEARTBEAT_JOB_NAME || isWithinActiveHours(nowMs),
      onJobDue: async (job: CronJob) => {
        ...
        try {
          ...
          console.warn("[cron] onJobDue failed:", (e as Error).message);
        } finally {
