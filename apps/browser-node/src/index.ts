...
import { webkit, type Page, type BrowserContext, type Browser } from "playwright";
...
async function handleBrowserFetch(params: Record<string, unknown>, timeoutMs: number) {
  ...
  if (!currentPageOnly && !url) {
    return { ok: false as const, error: { code: "INVALID_PARAMS", message: "browser_fetch requires params.url（或 currentPageOnly: true 仅截当前页）" } };
  }
  ...
  let waitUntil = (["domcontentloaded", "load", "networkidle"] as const).includes(
    ...
    ? (params.waitUntil as "domcontentloaded" | "load" | "networkidle")
    : "domcontentloaded";
  if (waitUntil === "domcontentloaded" && !currentPageOnly && url && /zhihu\.com\/people\/[^/]+\/(followers|following)/i.test(url)) {
  ...
  try {
    ...
    const out =
      text.length > MAX_FETCH_LENGTH ? text.slice(0, MAX_FETCH_LENGTH) + "\n\n[truncated]" : text;
...
ws.on("message", async (data: Buffer | ArrayBuffer) => {
  ...
  if (command === "browser_switch") {
    ...
    if (!persistentContext || !Number.isInteger(index) || index < 0) {
      sendResult(ws, invokeId, {
        ok: false,
        error: { code: "INVALID_PARAMS", message: "browser_switch 需要 persistent context 且 params.index 为有效非负整数" },
      });
      return;
    }
    const list = persistentContext.pages();
    if (index >= list.length) {
      sendResult(ws, invokeId, { ok: false, error: { code: "INVALID_PARAMS", message: `index ${index} 超出范围 (0-${list.length - 1})` } });
      return;
    }
  ...
  if (command === "browser_click") {
    try {
      ...
      const waitAfterMs = typeof params.waitAfterMs === "number" && params.waitAfterMs >= 0 ? Math.min(params.waitAfterMs, 60_000) : 0;
      if (selector) {
        ...
        sendResult(ws, invokeId, { ok: false, error: { code: "INVALID_PARAMS", message: "browser_click 需要 params.selector 或 params.text" } });
        return;
      }
      let screenshotBase64: string | undefined;
      if (waitAfterMs > 0) {
  ...
  if (command === "browser_fill") {
    try {
      ...
      if (typeof params.text !== "string") {
        sendResult(ws, invokeId, { ok: false, error: { code: "INVALID_PARAMS", message: "browser_fill 需要 params.text（要填入的内容，可为空字符串清空）" } });
        return;
      }
      ...
      if (selector) {
        ...
        sendResult(ws, invokeId, { ok: false, error: { code: "INVALID_PARAMS", message: "browser_fill 需要 params.selector 或 params.placeholder 指定目标输入框" } });
        return;
      }
      await new Promise((r) => setTimeout(r, 300));
...
const onFirstMessage = (data: Buffer | ArrayBuffer) => {
  ...
  if (msg.id === "connect-1") {
    ...
    console.log(
      `Browser Node 已连接: nodeId=${NODE_ID}, headed=${HEADED}, userDataDir=${USER_DATA_DIR ?? "none"}`,
    );
  }
};
