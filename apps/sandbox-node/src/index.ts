...
import { DEFAULT_WORKDIR, ensureSandboxContainer, getSandboxContainerName, runInContainer } from "./docker.js";
...
function sendResult(
	...
	if (result.ok) {
		const payloadJSON = result.payloadJSON ?? (result.payload != null ? JSON.stringify(result.payload) : undefined);
