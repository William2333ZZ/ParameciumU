...
| agent | 启动 Agent 的 app | 连 Gateway、注册 agentId/deviceId、收 node.invoke 派发、用 agent-from-dir 跑 runTurn、回传；含 heartbeat 定时（见 [heartbeat.md](./heartbeat.md)）。 |
...
- **推荐**：终端 1 起 `npm run gateway`，终端 2 起 `GATEWAY_URL=... AGENT_ID=.u npm run agent`，浏览器开 Control UI 或运行 TUI。
