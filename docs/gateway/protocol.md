...
本文档说明 ParameciumU Gateway 的协议、服务端与客户端分工、会话与 RPC、以及扩展要点。实现对应 `packages/gateway`（协议+客户端）与 `apps/gateway`（服务端）。协议为 ParameciumU 自有设计，不承诺与任何外部系统（如 OpenClaw）兼容；演进以 ParameciumU 需求为准。
...
- **agents.list**（返回含 `lastHeartbeatAt`，见 [Heartbeat](../automation/heartbeat.md)）、**sessions.list**
...
- **会话存储**：元数据文件为 `.gateway/sessions/sessions.json`，单条会话记录在 `.gateway/sessions/transcripts/&lt;sessionKey&gt;.json`。所有会话都有 sessionKey；未指定时按时间新建（如 `agent:.first_paramecium:s-&lt;timestamp&gt;-&lt;random&gt;`）。
...
- [应用与环境变量](../runtime/apps.md)
- [Agent 目录与运行](../concepts/agent-directory.md) · [Agent 运行机制](../runtime/agent-running.md)
