---
title: "Agent 运行机制"
summary: "Agent 目录加载、执行循环、Heartbeat 与 Cron 的代码级运行机制"
read_when:
  - 开发或排查 agent 执行、工具调用、心跳时
  - 理解 buildSession、runOneTurn、runScheduler 时
---

# Agent 运行机制

本文档根据当前代码整理 ParameciumU 中 **Agent 定义与目录**、**执行过程** 与 **Heartbeat** 的完整运行机制，供开发与排查参考。与 [architecture.md](../concepts/architecture.md)、[gateway.md](../gateway/protocol.md) 配合阅读。
...
任一 Agent 根目录（如 `.first_paramecium` 或 `agents/sidekick`）约定如下（与 `@monou/agent-template` 的 template 同构）：
...
- **路径约定**：`getAgentDir(rootDir)` 默认返回 `path.join(rootDir, ".first_paramecium")`；多 Agent 时通过环境变量 `AGENT_DIR` 或参数 `agentDir` 指定。
...
- Heartbeat 与 Cron：[heartbeat](./heartbeat.md) / [Heartbeat（自动化）](../automation/heartbeat.md)
- 整体架构：[architecture](../concepts/architecture.md)
- Gateway 与 agent.heartbeat：[gateway](../gateway/protocol.md)
